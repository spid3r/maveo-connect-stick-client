...
- **Branch flow** (same model as [loxberry-api-abfall-io](https://github.com/spid3r/loxberry-api-abfall-io)):
  ...
  - **`beta` → `main`** (PR): when ready, merge to ship stable; push to **`main`** → same workflow: stable release to npm **`latest`** + GitHub Release. Prefer a **merge commit** (not squash) for promotion PRs, or ensure the squash message does not include `[skip ci]` from prior release commits — otherwise CI/Release will not run until you push again.
